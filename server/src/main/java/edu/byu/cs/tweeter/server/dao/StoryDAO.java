...
public class StoryDAO {
    ...
    public StatusDTO addTweet(StatusDTO status){
        // add the new status to their story.
        AmazonDynamoDB client = AmazonDynamoDBClientBuilder.standard().build();

        UpdateItemRequest request = new UpdateItemRequest();
        request.setTableName("Story");
        request.setKey(Collections.singletonMap("Alias",
                new AttributeValue().withS(status.getUser())));
        request.setUpdateExpression("list_append(:prepend_value, UserStory)");
    ...
    public ArrayList<StatusDTO> getStory(String alias){
        ...
        GetItemSpec spec = new GetItemSpec()
                ...
                .withProjectionExpression("Alias, UserStory");
        Item item = table.getItem(spec);
        Map<String, Object> map = item.asMap();
        String user = (String)map.get("Alias");
        ArrayList<HashMap<String, Object>> story = (ArrayList<HashMap<String, Object>>) map.get("UserStory");
        ArrayList<StatusDTO> statuses = new ArrayList<>();
        for (HashMap<String, Object> status : story){
